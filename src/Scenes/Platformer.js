class Platformer extends Phaser.Scene {
  ...
  init() {
    ...
    this.WATER_JUMP_VELOCITY = -200;
    this.WATER_ACCELERATION = 250;
    this.WATER_DRAG = 800;
    this.waterTiles = [];
    ...
    this.collectedDiamonds = new Set();
  ...
  create() {
    ...
    this.cloudsLayer = this.map
      .createLayer("Clouds-n-Shit", this.tileset, 0, 0)
      .setScrollFactor(0.5)
      .setScale(SCALE);

    this.treesLayer = this.map
      .createLayer("Trees-n-Shrooms", this.tileset, 0, 0)
    ...
    this.coinVfxEffect = this.add.particles(0, 0, "tilemap_tiles", {
      frame: 151,
      emitting: false,
      lifespan: 1500,
    ...
    this.diamondVfxEffect = this.add.particles(0, 0, "tilemap_tiles", {
      frame: 67,
      emitting: false,
      lifespan: 1500,
      scale: { start: 1.0, end: 0.0 },
  ...
  createDiamonds() {
    ...
    this.diamonds
      .map((diamond) => {
        ...
        diamond.x = diamond.x * SCALE;
        diamond.y = diamond.y * SCALE;
        diamond.body.x = diamond.x;
        diamond.body.y = diamond.y;
        diamond.body.setCircle(5 * SCALE).setOffset(-5 * SCALE, -5 * SCALE);

        const diamondKey = `${diamond.x},${diamond.y}`;
        if (
          this.currentCheckpoint &&
          this.currentCheckpoint.collectedDiamonds.has(diamondKey)
        ) {
          diamond.destroy();
          return null;
        }
    ...
    this.physics.add.overlap(
      ...
      (obj1, obj2) => {
        if (obj2.active) {
          this.diamondVfxEffect.explode(3, obj2.x, obj2.y);
          this.sound.play("diamond_collect");
          const diamondKey = `${obj2.x},${obj2.y}`;
          this.collectedDiamonds.add(diamondKey);
          obj2.destroy();
        }
      }
    );
  ...
  createItemBoxes() {
    ...
    this.physics.add.collider(
      ...
      (player, box) => {
        ...
        const boxTop = box.body.y;
        const boxBottom = box.body.y + box.body.height;
  ...
  update() {
    ...
    if (
      ...
      this.currentCheckpoint = {
        ...
        collectedDiamonds: new Set(this.collectedDiamonds),
      };
  ...
  restartPlayer() {
    if (this.currentCheckpoint) {
      ...
      this.collectedDiamonds = new Set(
        this.currentCheckpoint.collectedDiamonds
      );
    } else {
    ...
    this.diamondGroup.clear(true, true);
    this.itemBoxGroup.clear(true, true);
